...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT) then
        ...
        context:runAnimation("random_angle",context:getTrack(STATIC_TRACK_LINE, RANDOM_TRACK_A),false,LOOP,0)
        context:runAnimation("random_scale",context:getTrack(STATIC_TRACK_LINE, RANDOM_TRACK_B),false,LOOP,0)
        local a = math.random()
        local b = math.random()
