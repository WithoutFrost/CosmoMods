...
local main_track_states = {
    ...
    reload_left = {current_bullet = 8, is_left = true},
    reload_right = {current_bullet = 8, is_left = false},--左右换弹
    -- 刺刀攻击的计数器
    bayonet_counter = 0
}
...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_RELOAD) then
        local ammo_count = context:getAmmoCount()
        if (ammo_count >= 9) then
            context:trigger(this.INPUT_RELOAD_LEFT)
        else
            context:trigger(this.INPUT_RELOAD_RIGHT)
        end
        return this.main_track_states.idle
...
function main_track_states.reload_left.entry(this, context)
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    context:runAnimation("reload_left_start", track, false, PLAY_ONCE_HOLD, 0.2)
end

function main_track_states.reload_left.update(this, context)
    local ammo_count = context:getAmmoCount()
    if (ammo_count > 0 and ammo_count < 9) then
        context:trigger(this.INPUT_RELOAD_LEFT)
    elseif (ammo_count == 9) then
        context:trigger(this.INPUT_POSE_CONVERSION)
    end
end

function main_track_states.reload_left.transition(this, context, input)
    if (input == this.INPUT_RELOAD_LEFT) then
        context:runAnimation("reload_left", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.idle
    elseif (input == this.INPUT_POSE_CONVERSION) then
        local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
        context:runAnimation("pose_conversion", track, false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.idle
    end
    return nil
end
...
-- 装填右管状态
function main_track_states.reload_right.entry(this, context)
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    context:runAnimation("reload_right_start", track, false, PLAY_ONCE_HOLD, 0.2)
end

function main_track_states.reload_right.update(this, context)
    local ammo_count = context:getAmmoCount()
    if (ammo_count > 0 and ammo_count < 9) then
        context:trigger(this.INPUT_RELOAD_RIGHT)
    elseif (ammo_count == 9) then
...
function main_track_states.reload_right.transition(this, context, input)
    if (input == this.INPUT_RELOAD_RIGHT) then
        context:runAnimation("reload_right", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        return this.main_track_states.idle
    elseif input == this.INPUT_POSE_CONVERSION then
        local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
        context:runAnimation("pose_conversion", track, false, PLAY_ONCE_STOP, 0.2)
...
local M = {
 ...
 -- 新增状态
 reload_left = main_track_states.reload_left,
 reload_right = main_track_states.reload_right,
 -- 新增输入
 INPUT_RELOAD_LEFT = "reload_left",
 INPUT_RELOAD_RIGHT = "reload_right",
 INPUT_POSE_CONVERSION = "pose_conversion",
