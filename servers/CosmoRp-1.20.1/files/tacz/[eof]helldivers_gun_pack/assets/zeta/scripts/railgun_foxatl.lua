...
function bolt_caught_states.normal.update(this, context)
    ...
    if (isNoAmmo(context)) then
        ...
        if ((bolt_caught_states.trigger)and(t2 - t1 > 2*itv+edge)) then
            local track = context:findIdleTrack(GUN_KICK_TRACK_LINE, false)
            if bolt_caught_states.proc>=10 then
            ...
            bolt_caught_states.trigger = false
            bolt_caught_states.proc = 0
        end
    a = math.min(((bolt_caught_states.proc)/max),1)
    context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK),a,true)
end
    ...
    local track = context:findIdleTrack(GUN_KICK_TRACK_LINE, false)
        ...
        if (input == INPUT_SHOOT) then
            ...
            if (t2 - t1 <= itv+edge) then
                local mark = bolt_caught_states.proc
                if (mark) <=0 then
                    context:runAnimation("charge_start", track, true, PLAY_ONCE_STOP, 0)--charge_start里只放开始蓄力的音效
                end
                -- 没有松手（宽限为edge/ms）
                if bolt_caught_states.proc > max then
                    context:runAnimation("shoot", track, true, PLAY_ONCE_STOP, 0)
                    bolt_caught_states.proc = max
                    bolt_caught_states.trigger = false
                else
                    bolt_caught_states.proc = bolt_caught_states.proc + step
                    bolt_caught_states.trigger = true
                end
            else
                bolt_caught_states.proc = 0
                bolt_caught_states.trigger = false
