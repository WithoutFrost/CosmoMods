...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT) then
        ...
        if (context:isStopped(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK))) then
            context:runAnimation("shoot_1", context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), false, PLAY_ONCE_STOP, 0)
        end
    end
    return nil
end
